...
class Neuron:
    ...
    def __call__(self, x):
        ...
        # tanh(w * x + b)
        paired = zip(self.w, x)
        activationVal = sum((wi*xi for wi, xi in paired), self.b)
        out = activationVal.tanh()
...
def binaryClassifier():
    ...
    for k in range(iterations):
        ...
        # backward pass
        loss.backward_prop()                                                    # running backpropagation to get gradients of all the neurons relative to the summed loss

        # gradient descent
        for p in n.parameters():
            # print('data: ', p.data)
            # print('grad:', p.grad)
            p.data += -0.05 * p.grad    # it is '+= -0.05' relationship because the cost function is mean squared error, cost will always be positive meaning target loss is 0
